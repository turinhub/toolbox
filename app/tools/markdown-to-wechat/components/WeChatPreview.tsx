...
export const WeChatPreview = forwardRef<WeChatPreviewHandle, WeChatPreviewProps>(
      ...
      >
        <ReactMarkdown
          ...
          components={{
            ...
            code: ({ isBlock, style, className, children, ...props }: CodeProps) => {
              if (isBlock) {
                const codeContent = String(children).replace(/\n$/, "");
                if (config.codeLineNumber) {
                   const lines = codeContent.split('\n');
                   return (
                     <div style={{ display: 'flex', fontFamily: 'inherit', ...style }} className={className} {...props}>
                       <div style={{ 
                         textAlign: 'right', 
                         paddingRight: '1em', 
                         userSelect: 'none', 
        ...
        {config.wechatLink && links.length > 0 && (
          <div style={{ marginTop: "2em", paddingTop: "1em", borderTop: "1px solid #eee" }}>
            <h4 style={{ fontSize: "1em", fontWeight: "bold", marginBottom: "0.5em" }}>引用链接</h4>
            <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
              {links.map((link, index) => (
                <li key={index} style={{ fontSize: "0.9em", color: "#666", marginBottom: "0.3em" }}>
                  [{index + 1}] {link.text}: <span style={{ wordBreak: "break-all" }}>{link.href}</span>
                </li>
              ))}
            </ul>
          </div>
