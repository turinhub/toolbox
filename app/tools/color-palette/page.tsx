    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <Tabs defaultValue="palettes" className="space-y-6">
        <TabsList className="grid w-full grid-cols-3">
          ...
          <TabsTrigger value="custom">自定义配色</TabsTrigger>
        </TabsList>
          ...
          <Card className="p-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-medium mb-2 block">输入颜色值</label>
                  <div className="flex gap-2">
                    <Input
                      type="color"
                      value={inputColor}
                ...
                <Button
                  onClick={() => addToCustomPalette(inputColor)}
                  variant="outline"
                  className="w-full"
                >
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <span className="font-medium">HEX:</span> {colorAnalysis.hex}
                      </div>
                      <div>
                        <span className="font-medium">RGB:</span> {colorAnalysis.rgb.r}, {colorAnalysis.rgb.g}, {colorAnalysis.rgb.b}
                      </div>
                      <div>
                        <span className="font-medium">HSL:</span> {colorAnalysis.hsl.h}°, {colorAnalysis.hsl.s}%, {colorAnalysis.hsl.l}%
                      </div>
                    </div>
                    ...
                    <div className="pt-2 border-t">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex gap-2 mt-2">
                          <Badge variant={colorAnalysis.wcagAA ? "default" : "destructive"}>
                            WCAG AA {colorAnalysis.wcagAA ? "✓" : "✗"}
                          </Badge>
                          <Badge variant={colorAnalysis.wcagAAA ? "default" : "destructive"}>
        ...
        <TabsContent value="custom" className="space-y-6">
          <Card className="p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">自定义配色方案</h3>
              <div className="flex gap-2">
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => copyPalette(customColors)}
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => exportPalette(customColors, "Custom Palette")}
                  disabled={customColors.length === 0}
                >
                  <Download className="h-4 w-4 mr-1" />
                  导出
            ...
            {customColors.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p>还没有添加任何颜色</p>
                <p className="text-sm">在颜色分析器中添加颜色到自定义配色</p>
              </div>
            ) : (
              <div className="grid grid-cols-5 md:grid-cols-10 gap-2">
      ...
      <div className="mt-8">
        ...
        <ul className="list-disc list-inside space-y-2 text-muted-foreground">
          <li><strong>配色方案:</strong> 浏览各种经典配色方案，点击颜色块复制颜色值</li>
          <li><strong>颜色分析:</strong> 输入颜色值获取详细的颜色信息和可访问性分析</li>
          <li><strong>自定义配色:</strong> 创建和管理你自己的配色方案</li>
          <li><strong>导出功能:</strong> 将配色方案导出为 JSON 文件，便于在其他项目中使用</li>
          <li><strong>对比度检测:</strong> 自动检测颜色是否符合 WCAG 可访问性标准</li>
        </ul>
      </div>
    </div>
  );
