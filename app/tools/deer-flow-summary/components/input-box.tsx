...
import { BorderBeam } from "@/components/deer-flow/magicui/border-beam";
...
export function InputBox({
  ...
  return (
    <div
      className={cn(
        "bg-card relative flex h-full w-full flex-col rounded-[20px] border",
        className,
      )}
      ref={containerRef}
    >
      ...
      <div className="flex items-center px-4 pb-1 pt-0">
        <div className="flex grow gap-1">
          {config?.models.reasoning?.[0] && (
            ...
            >
              <Button
                className={cn(
                  "rounded-xl py-0.5 px-2 text-xs transition-all duration-200",
                  enableDeepThinking 
                    ? "!border-brand !bg-brand !text-white shadow-md" 
                    : "border-gray-300 hover:border-gray-400",
                )}
                variant={enableDeepThinking ? "default" : "outline"}
                onClick={() => {
          ...
          >
            <Button
              ...
              variant={backgroundInvestigation ? "default" : "outline"}
              onClick={() =>
