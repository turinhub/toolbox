...
export function MessageListView({
  ...
  useEffect(() => {
    const scrollToBottom = (force = false) => {
      // Primary method: use the scroll container ref
      if (scrollContainerRef.current) {
        scrollContainerRef.current.scrollToBottom();
      }
      
      // Fallback method: find and scroll the actual scroll container
      const fallbackScroll = () => {
        const scrollAreas = document.querySelectorAll('[data-radix-scroll-area-viewport]');
        scrollAreas.forEach(area => {
          if (area.scrollTop !== undefined) {
    ...
    if (typeof window !== 'undefined') {
      ...
      // Try to find the scroll container more reliably
      const findScrollContainer = () => {
        return scrollContainerRef.current?.scrollRef?.current?.querySelector('[data-radix-scroll-area-viewport]') ||
    ...
    // Aggressive scrolling strategy with multiple approaches
    const aggressiveScroll = () => {
      // Try multiple scroll methods
      scrollToBottom();
      
      // Immediate retries with increasing delays
      const immediateRetries = [0, 10, 50, 100, 200, 300, 500, 800, 1000, 1500];
      immediateRetries.forEach(delay => {
        setTimeout(() => scrollToBottom(true), delay);
      });
      
      // Force scroll after animations complete
      setTimeout(() => {
        const scrollAreas = document.querySelectorAll('[data-radix-scroll-area-viewport]');
        scrollAreas.forEach(area => {
    ...
    // Execute scrolling immediately
    aggressiveScroll();
    
    // Continuous scroll during streaming for live content
    let streamingInterval: NodeJS.Timeout | null = null;
    if (responding) {
      streamingInterval = setInterval(() => scrollToBottom(true), 150);
    }
