...
export function ResearchActivitiesBlock({
  ...
  useEffect(() => {
    ...
    // Aggressive scrolling strategy for activities
    const aggressiveScroll = () => {
      scrollToBottom();
      
      // Multiple retries for reliable scrolling
      const retries = [0, 50, 150, 300, 500, 800];
      retries.forEach(delay => {
        setTimeout(() => scrollToBottom(true), delay);
      });
    };

    // Execute immediate scrolling
    aggressiveScroll();
    
    // Continuous scroll during ongoing research
    let streamingInterval: NodeJS.Timeout | null = null;
    if (ongoing) {
      streamingInterval = setInterval(() => scrollToBottom(true), 300);
    }
...
function RetrieverToolCall({ toolCall }: { toolCall: ToolCallRuntime }) {
  ...
  >(() => {
    try {
      ...
      console.warn("Failed to parse documents:", error);
      return [];
    }
  }, [toolCall.result]);
  return (
