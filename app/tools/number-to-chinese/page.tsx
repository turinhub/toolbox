...
// 数字转中文大写函数
const numberToChinese = (num: number | string, withPrefix = false, simpleMode = false): string => {
  // 数字映射
      ...
      if (digit === 0) {
        ...
        if (position === 8 || position === 4) {
          if (integerStr.length > 0) {
            // 判断是否已经有了亿、万单位
            if ((position === 8 && !integerStr.includes('亿')) || 
                (position === 4 && !integerStr.includes('万') && parseInt(integerPart.substring(0, integerLen - 8)) === 0)) {
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="simple-mode">使用简体字形（一二三 代替 壹贰叁）</Label>
          </div>
          
          <div className="flex flex-col space-y-4">
            <Input
              type="text"
              placeholder={mode === "currency" ? "请输入数字金额，如：1234.56" : "请输入数字，如：1234"}
              value={input}
            ...
            {!validInput && (
              <Alert variant="destructive">
                <AlertDescription>请输入有效的数字格式，如：123 或 123.45</AlertDescription>
              </Alert>
            )}
        ...
        <CardContent>
          <div className="grid grid-cols-1 gap-4">
            {examples.map((example, index) => (
              ...
              >
                <div className="font-medium">{example.number} → {example.chinese}</div>
