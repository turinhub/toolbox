...
import { AlertCircle, CheckCircle2, Save, Upload, X, Eye, EyeOff, Copy, Check, ChevronsUpDown } from 'lucide-react';
...
export default function OpenAICheckerPage() {
  ...
  const [savedConfigs, setSavedConfigs] = useState<{name: string, config: OpenAIConfig}[]>([]);
  ...
  const saveConfig = () => {
    ...
    // 检查是否存在同名配置
    const existingIndex = savedConfigs.findIndex(item => item.name === configName);
    const newConfig = {
      name: configName,
      config: { 
        endpoint, 
  ...
  const loadConfig = (config: OpenAIConfig, configName?: string) => {
    ...
    setUseTemperature(config.useTemperature !== undefined ? config.useTemperature : true);
    setUseMaxTokens(config.useMaxTokens !== undefined ? config.useMaxTokens : true);
    
    // 更新模型输入值
    const selectedOption = modelOptions.find(option => option.value === config.model);
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          updateTestResults('API 连接测试', 'success', `连接成功，${usedTokens}`, data);
          toast.success('OpenAI 接口连接成功');
        } catch (fetchError) {
          // 处理 fetch 错误
          if (fetchError instanceof TypeError && fetchError.message.includes('Failed to fetch')) {
            // 设置一个友好的错误消息
            const corsMessage = '这可能是由于跨域 (CORS) 限制导致的。浏览器的安全策略阻止了从一个源访问另一个源的资源。';
            const networkMessage = '或者可能是网络连接问题，请检查您的网络连接是否正常。';
            const endpointMessage = '请确保您的 Endpoint URL 格式正确，包含协议(http/https)和正确的端口。';
            const proxyMessage = '如果您在使用代理，请确保代理配置正确。';
            
            const errorMsg = `网络请求失败 (Failed to fetch):\n\n${corsMessage}\n\n${networkMessage}\n\n${endpointMessage}\n\n${proxyMessage}\n\n请尝试以下解决方案:\n1. 使用支持 CORS 的 API 端点\n2. 使用代理服务器转发请求\n3. 检查网络连接\n4. 确认 API 端点是否可用`;
            
            setResponse(`请求失败: Failed to fetch\n\n请求 URL: ${apiUrl}\n\n${errorMsg}`);
        ...
        <TabsContent value="connection" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>请输入您的 OpenAI 兼容接口配置信息</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label>API Endpoint <span className="text-red-500">*</span></Label>
                <div className="flex">
                  <Input
                  ...
                  <Button 
                    ...
                    disabled={!endpoint}
                  >
                    {copyState['endpoint'] ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                  </Button>
                </div>
                {endpointError && (
              ...
              <div className="space-y-2">
                ...
                <div className="flex">
                  ...
                  <Button 
                    ...
                    disabled={!apiKey}
                  >
                    {copyState['apiKey'] ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                  </Button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label>模型 <span className="text-red-500">*</span></Label>
                        ...
                        <div className="p-1">
                          <div className="border-t mt-1 pt-1">
                            <Button
                              ...
                              onClick={() => {
                                if (modelInputValue && !modelOptions.some(option => option.label === modelInputValue)) {
                                  setModel(modelInputValue);
                                }
                                setOpenModelSelector(false);
                              }}
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      />
                      <Label htmlFor="use-temperature" className="text-xs text-muted-foreground">启用</Label>
                    </div>
                  </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      />
                      <Label htmlFor="use-max-tokens" className="text-xs text-muted-foreground">启用</Label>
                    </div>
                  </div>
                  <Input
                    type="number"
              ...
              <CardContent>
                <div className="space-y-3">
                  {testResults.map((result, index) => (
                    <div key={index}>
                      <Alert className={
                        result.status === 'success' ? "border-green-500 bg-green-50 dark:bg-green-950/20" :
                        result.status === 'error' ? "border-red-500 bg-red-50 dark:bg-red-950/20" :
                        ...
                        {result.status === 'pending' && <div className="h-4 w-4 rounded-full border-2 border-yellow-500 border-t-transparent animate-spin" />}
                ...
                {response && (
                  <div className="mt-4">
                    ...
                    <div className="flex justify-end mt-2">
                      ...
                      >
                        {copyState['response'] ? <Check className="h-4 w-4 mr-1" /> : <Copy className="h-4 w-4 mr-1" />}
                        复制响应
                      </Button>
                    </div>
                  </div>
                ...
                {testResults.some(r => r.status === 'error') && (
                  <div className="mt-4 p-4 border border-yellow-500 rounded-md bg-yellow-50 dark:bg-yellow-950/20">
                    ...
                    <ul className="list-disc pl-5 space-y-1 text-sm">
                      <li>确保 API Endpoint URL 格式正确，包含协议（http:// 或 https://）</li>
                      <li>检查 API Key 是否正确</li>
                      <li>确认所选模型在您的 API 提供商中可用</li>
                      <li>如果遇到跨域问题，需要在 API 服务端配置 CORS 策略</li>
                      <li>检查网络连接是否正常，特别是在使用私有网络或 VPN 时</li>
                      <li>如果使用第三方 API 服务，确认其是否完全兼容 OpenAI 接口</li>
                    </ul>
                    
                    {testResults.some(r => r.message?.includes('Failed to fetch')) && (
                      <div className="mt-4">
                        <h3 className="font-medium mb-2">解决 &ldquo;Failed to fetch&rdquo; 错误：</h3>
                        <div className="bg-white dark:bg-gray-800 p-3 rounded-md border border-gray-200 dark:border-gray-700 text-sm">
                          <p className="mb-2 font-medium">这个错误通常是由于浏览器无法连接到 API 服务器导致的，常见原因包括：</p>
                          <ol className="list-decimal pl-5 space-y-1">
                            <li><span className="font-medium">跨域 (CORS) 限制</span> - 浏览器的安全策略阻止了从一个源访问另一个源的资源</li>
                            <li><span className="font-medium">网络连接问题</span> - 您的网络可能无法访问 API 服务器</li>
                            <li><span className="font-medium">API 端点不可用</span> - 服务器可能已关闭或不接受连接</li>
                            <li><span className="font-medium">防火墙或安全设置</span> - 可能阻止了对 API 服务器的访问</li>
                          </ol>
                          
                          <p className="mt-3 mb-2 font-medium">可能的解决方案：</p>
                          <ol className="list-decimal pl-5 space-y-1">
                            <li>
                              <span className="font-medium">使用代理服务器</span> - 在服务器端发起请求可以绕过 CORS 限制
                              <ul className="list-disc pl-5 mt-1 text-xs">
                                <li>在本工具中启用&ldquo;使用代理&rdquo;选项（需要后端支持）</li>
                                <li>或使用自己的代理服务器转发请求</li>
                              </ul>
                            </li>
                            <li>
                              <span className="font-medium">使用支持 CORS 的 API 端点</span>
                              <ul className="list-disc pl-5 mt-1 text-xs">
                                <li>确认您的 API 提供商是否支持跨域请求</li>
                                <li>如果您自己控制 API 服务器，添加适当的 CORS 头</li>
                              </ul>
                            </li>
                            <li>
                              <span className="font-medium">检查网络连接</span>
                              <ul className="list-disc pl-5 mt-1 text-xs">
                                <li>尝试在浏览器中直接访问 API 端点（可能会返回 401 错误，但至少表明端点可达）</li>
                                <li>检查您的网络是否有特殊限制或防火墙规则</li>
                              </ul>
                            </li>
                            <li>
                              <span className="font-medium">使用其他 API 客户端测试</span>
                              <ul className="list-disc pl-5 mt-1 text-xs">
                                <li>使用 Postman、curl 或其他 API 工具测试连接</li>
                                <li>如果其他工具可以连接，则问题可能是浏览器特定的</li>
                              </ul>
                            </li>
                          </ol>
                        </div>
        ...
        <TabsContent value="configs">
          <Card>
            <CardHeader>
              ...
              <CardDescription>加载、查看或管理您保存的 OpenAI 配置</CardDescription>
            </CardHeader>
            <CardContent>
                      ...
                      <div className="p-3 text-sm space-y-2 bg-muted/10">
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            ...
                            <span className="ml-1 break-all">{item.config.endpoint}</span>
                          </div>
                          <div>
                            <span className="font-medium">模型:</span> 
                            <span className="ml-1">{modelOptions.find(option => option.value === item.config.model)?.label || item.config.model}</span>
                          </div>
                        </div>
                        <div>
                          <span className="font-medium">API Key:</span> 
                          <span className="ml-1">{item.config.apiKey.substring(0, 3)}***{item.config.apiKey.substring(item.config.apiKey.length - 3)}</span>
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            <span className="font-medium">Temperature:</span> 
                            <span className="ml-1">{item.config.temperature}</span>
                          ...
                          <div>
                            ...
                            <span className="ml-1">{item.config.maxTokens}</span>
                        ...
                        <div>
                          ...
                          <span className="ml-1">{item.config.useProxy ? '是' : '否'}</span>
                        </div>
                      </div>
                    </div>
                  ))}
