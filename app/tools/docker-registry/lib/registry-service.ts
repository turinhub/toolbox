  ...
  if (res.status === 401) {
    ...
    if (authHeader && authHeader.toLowerCase().startsWith("bearer")) {
      ...
      if (params.realm) {
        ...
        if (params.service) tokenUrl.searchParams.append("service", params.service);
...
export async function getTags(config: RegistryConfig, repository: string) {
    try {
        const baseUrl = config.url.replace(/\/$/, "");
        const res = await fetchWithAuth(`${baseUrl}/v2/${repository}/tags/list`, config);
...
export async function getManifest(config: RegistryConfig, repository: string, reference: string) {
    try {
        const baseUrl = config.url.replace(/\/$/, "");
        ...
        const res = await fetchWithAuth(`${baseUrl}/v2/${repository}/manifests/${reference}`, config, { headers });
...
export async function deleteManifest(config: RegistryConfig, repository: string, digest: string) {
    try {
        const baseUrl = config.url.replace(/\/$/, "");
        const res = await fetchWithAuth(`${baseUrl}/v2/${repository}/manifests/${digest}`, config, {
            method: "DELETE"
        });
